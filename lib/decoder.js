...
/**
 * Convert a byte-buffer representing an integer into a nativ JS Number.
 *
 * Remember: values x such that -2^31 > x or x > 2^31-1 will make this routine
 * puke.
 *
 * @param {Buffer} bytes A Buffer representing an integer.
 * @return {Number} The decoded number.
 */
var bytesToNum = module.exports.bytesToNum = function(bytes) {
  var num = 0;
  // If the sign bit is on, start with every bit asserted. We only care about
  // 32 bits because we lose precision after that anyway.
...
/**
 * Convert a byte-buffer representing an integer into a BigInteger.
 *
 * @param {Buffer} bytes A Buffer representing an integer.
 * @return {BigInteger} The decoded number.
 */
var bytesToBigInt = module.exports.bytesToBigInt = function(bytes) {
  // Convert bytes (which is really a string) to a list of ints.
  // Then convert the bytes (ints) to a BigInteger.
  var ints = [];
  for (var i = 0; i < bytes.length; i++) {
    // how does this handle negative values (bytes that overflow 127)?
    ints[i] = bytes[i];
    if (ints[i] > 255) {
...
/**
 * Convert an 8 byte buffer representing an integer into a BigInteger.
 *
 * @param {Buffer} bytes A Buffer representing an integer.
 * @return {BigInteger} The decoded number.
 */
var bytesToBigLong = module.exports.bytesToBigLong = function(bytes) {
  ...
  // Trim all leading zeros except the most significant one (we don't want to
  // flip signs).
...
/**
 * Decoder for cassandra resultsets
 *
 * Validators are a hash currently created in the Connection constructor.
 * Keys in the hash are: key, comparator, defaultValidator, specificValidator.
 * They all map to a value in AbstractTypes, except specificValidator which
 * hashes to another map that maps specific column names to their validators
 * (specified in ColumnDef using Cassandra parlance).
 *
 * e.g.: {
 *   key: 'org.apache.cassandra.db.marshal.BytesType',
 ...
 *
 * Options are: {
 *   use_bigints: true|false, // default: false @see driver.js
 *   select_count: true|false // default: false, internal usage only
 * }
 *
 * @param {Object} validators Validators as described above.
...
/**
 * Decodes byte buffer according to the specified validator.
 *
 * @param {Buffer} bytes raw bytes to decode.
 * @param {string} which one of 'key', 'comparator', or 'value'.
 * @param {string} column (optional) when which is 'value' this parameter
 *     specifies which column validator is to be used.
 */
