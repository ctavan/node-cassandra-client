...
// Encapsulates a connection (thrift protocol) and a client bound to it.
...
// A pool of thrift connections. Keeping this class around because driver.js
// will eventually use something like this.
...
var System = module.exports.System = function(urn) {
  ...
  this.on('checkq', function() {
    ...
    if (!con) {
      // No connection is available. Create a timer event to check back in a
      // bit to see if there is a connection available to do work.
      setTimeout(function() {
        self.emit('checkq');
      }, 25);
    } else {
